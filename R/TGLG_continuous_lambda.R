...
#' @import truncnorm
#' @import Matrix
TGLG_continuous_lambda = function(X, y, net=NULL,nsim=30000, ntune=10000, freqTune=100,
  ...
  for(sim in 1:nsim){
    ...
    if(sim %in% iter){
      ...
      likelihood2[idx] = -sum((y-X%*%beta)^2)/(sigmae^2)*0.5
    }
  ...
  save_mcmc = cbind(iter,Beta,Alpha,Gamma,
                    Sigma.gamma,Sigma.alpha,Epsilon,Sigmae,
                    likelihood, likelihood2)
  colnames(save_mcmc) = c("iter",
                          paste("beta",1:p,sep=""),
                          paste("alpha",1:p,sep=""),
                          paste("gamma",1:p,sep=""),
                          "sigma_gamma","sigma_alpha","epsilon","sigmae","loglik","loglik2")
